...
func (h *handler) Render(path string, genExec *gengokit.Executor) (io.Reader, error) {
	if path != ServerFile {
		return nil, errors.Errorf("cannot render unknown file: %q", path)
	}
	if h.ast == nil {
		return applyServerTempl(genExec)
	}
	...
	h.ast.Decls = h.mMap.pruneDecls(h.ast.Decls, genExec.PackageName)
	...
	ex := handlerExecutor{
		PackageName: genExec.PackageName,
	}
...
func applyServerTempl(executor *gengokit.Executor) (io.Reader, error) {
	log.Debug("Rendering handler for the first time")
	return executor.ApplyTemplate(serverTempl, "ServerHandler")
}
